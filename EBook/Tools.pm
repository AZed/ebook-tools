...
=head2 add_document($href,$id,$mediatype)
...
=head2 add_item($href,$id,$mediatype)
...
they might link to, adding anything missing to the manifest.  This
method is called from inside fix_manifest().
...
fix_packageid(), and fix_links().

The objective here is that you can run either fix_misc() and either
fix_oeb12() or fix_opf20() and a perfectly valid OPF file will result.
...
=item * Makes sure that all links needed by files in the manifest
exist in the manifest via fix_links()
...
sub manifest
{
    ...
    @elements = $manifest->children('item');
...
twigroot, and the twigroot is <package>.
...
sub twigcheck
{
    ...
    croak("$calledfrom[3](): twigcheck found undefined twig")
        if(!$$self{twig});
    croak("$calledfrom[3](): twigcheck found twig with no root")
        if(!$$self{twigroot});
    croak("$calledfrom[3](): twigcheck found twig root '" 
          . $$self{twigroot}->gi . "' (needs to be 'package')")
