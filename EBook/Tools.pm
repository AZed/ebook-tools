...
sub init
{
    ...
    if($filename) { $self->set_opffile($filename); }
...
Unless otherwise specified, all methods return false if an error was
...
Adds @errors to the list of object warnings.  Each member of @errors
should be a string with no ending newline.

SEE ALSO: clear_errors, clear_warnerr, warn
...
SEE ALSO: error, clear_warnings, clear_warnerr
...
sub fixmisc ()
{
	...
	if(!$dcdate->text)
	{
	    $self->warn("WARNING: found dc:date with no value -- skipping");
	}
	else
	{
	    $newdate = fix_date($dcdate->text);
	    if(!$newdate)
	    {
		$self->warn(
		    sprintf("fixmisc(): the date '%s' is so badly formatted I can't deal with it -- skipping.",$dcdate->text)
		    );
	    }
	    else
	    {
...
sub gen_epub
{
    ...
    if(! $$self{opffile} )
    {
	error("Cannot create epub without an OPF (did you forget to init?)");
	return 0;
    }
    if(! -f $$self{opffile} )
    {
	error(sprintf("OPF '%s' does not exist (did you forget to save?)",
	      $$self{opffile}));
	return 0;
    }
...
sub save ()
{
    ...
    {
	error(sprintf("Could not open '%s' to save to!",$self->{opffile}));
	return 0;
    }
    ...
    if(!close(OPFFILE))
    {
	error(sprintf("Failure while closing '%s'!",$self->{opffile}));
	return 0;
    }
    return;
}
...
sub fix_date
{
	...
	{
	    $fixeddate = $year;
	    $fixeddate .= sprintf("-%02u",$month) if($month);
	    $fixeddate .= sprintf("-%02u",$day) if($day);
...
sub twig_fix_mobi ( $ )
{
    ...
    print "DEBUG: returning from twig_fix_mobi" if($debug > 1);
    return;
}
